...
`fixture-gen` Phase 9 adds uniqueness guarantees, cross-field refinement hooks, and business-rule hooks for relational generation.
...
// Ensure discountedPrice is always less than price
...
const { users, invoices } = generateRelational(
  { users: User, invoices: Invoice },
  {
    ...
    rules: [
      // free-plan users must have zero-amount invoices
      (tables) => {
