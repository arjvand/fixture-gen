...
**Current state: Phase 9 (Advanced constraint engine) completed.** `generate(schema, { seed })` produces deterministic, validating output for Zod, Valibot, ArkType, and TypeBox. `generateMany`, `generateRelational`, and scenario-first generation are also covered. Phase 9 adds: `unique` option for cross-record field uniqueness in `generateMany`, `refine` hook for cross-field invariants, `rules` hooks for `generateRelational`, and automatic deduplication for arrays with `uniqueItems`/`z.set()`. A standalone `fixture-gen` CLI ships with `generate`, `snapshot`, `diff`, and `watch` subcommands for drift detection and CI integration. The full stable API is specified in `README.md`, and the ordered build plan in `docs/ROADMAP.md`. `fixture-gen` is published to npm at version `1.2.0`.
